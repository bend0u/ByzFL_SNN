...
def get_best_test_accuracy(base_dir, pattern):
    ...
    accs = []
    for d in dirs:
        # Match test and val files directly by assuming they correspond or exist
...
def plot_for_gamma(gamma, ax):
    attacks_display = ['Baseline (f=0)', 'ALittleIsEnough', 'SignFlipping', 'InnerProductManipulation']
    attack_patterns = {
        'Baseline (f=0)': ('_f_0_', '*'), # For f=0, attack doesn't matter, average all
        'ALittleIsEnough': ('_f_5_', '*Optimal_ALittleIsEnough_neg1*'),
        'SignFlipping': ('_f_5_', '*SignFlipping*'),
        'InnerProductManipulation': ('_f_5_', '*Optimal_InnerProductManipulation*')
    }
    
    aggregators = ['Average', 'GeometricMedian', 'CenteredClipping', 'TrMean', 'MultiKrum']
    accuracies = {agg: [] for agg in aggregators}
    
    base_dir = 'results/cnn_dropout_experiment/'
    ...
    ax.set_title(f'CNN 93% Dropout Robustness (gamma={gamma})', fontsize=14)
...
# Put a single legend at the top of the figure
handles, labels = ax1.get_legend_handles_labels()
fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 0.98), ncol=5, fontsize=12)

plt.tight_layout(rect=[0, 0, 1, 0.93])

save_path = '/localhome/bendouro/.gemini/antigravity-ide/brain/881c0a4f-bb2b-4e36-be90-1f4adf0775dd/cnn_dropout_experiment_results.png'
