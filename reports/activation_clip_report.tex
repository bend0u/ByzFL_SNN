...
\subsection{Self-Calibrated Warmup Clip (\texttt{self\_grad\_clip\_warmup})}
Not an activation function -- a plain-ReLU CNN where each honest client bootstraps its
own \emph{absolute} raw-gradient-norm cap from its own early training, with no offline
probe, no SNN, and no continuously-adaptive statistic. For the first $W$ steps the
client only observes its raw gradient norm $n_t = \lVert g_t \rVert_2$ (no clipping);
once warmup ends the cap is frozen and applied unchanged thereafter:
\begin{align*}
    C_{\text{self}} &= \Big(\max_{t \le W} n_t\Big)\cdot m
    \qquad\text{(frozen at step $W$; $m$ = safety margin)} \\
    g_t^{\text{clipped}} &=
    \begin{cases}
        g_t \cdot \dfrac{C_{\text{self}}}{n_t} & t > W \ \text{and}\ n_t > C_{\text{self}} \\
        g_t & \text{otherwise}
    \end{cases}
\end{align*}
Applied to the raw gradient (same position as \texttt{gradient\_clip\_val}$=21$). A
short offline probe of the honest \texttt{cnn\_mnist} shows the raw grad-norm ramps up
$4.5\times$--$8\times$ over the first $\sim$25--60 steps (heterogeneity transient)
before decaying, so $W$ matters: $W=1$ (``clip to the literal first gradient'') freezes
a cap well below the true honest peak, whereas $W\approx 50$--$75$ captures the peak and
converges to the offline-calibrated honest ceiling.

\paragraph{Pilot results (GeometricMedian only, 3 seeds).} \texttt{best\_test} (worst
case across attacks) for $W=1$, $m=1.0$:
\begin{center}
\begin{tabular}{c|cccccc}
$\gamma \backslash f$ & 0 & 1 & 2 & 3 & 4 & 5 \\ \hline
...
\end{tabular}
\end{center}
Same pilot for $W=50$, $m=1.0$ (cap $\approx$ the honest ceiling, essentially
zero honest clipping -- see the offline probe analysis above):
\begin{center}
\begin{tabular}{c|cccccc}
$\gamma \backslash f$ & 0 & 1 & 2 & 3 & 4 & 5 \\ \hline
...
\paragraph{Finding -- the prediction above was wrong, in the opposite direction.} $W=1$
was expected to collapse at high heterogeneity from an under-sized cap; instead it
\emph{dramatically outperforms} $W=50$ at $\gamma=0.33$ (+18 to +43 accuracy points
across every $f$) and clearly ahead at $\gamma=0$, $f=1$ ($0.81$ vs $0.16$). $W=50$ is
only slightly ahead at low heterogeneity ($\gamma \in \{1.0, 0.66\}$, +2 to +5 points),
where the extra clipping was never needed. Both are tied at the pre-existing
$\gamma=0$, $f\ge 2$ wall ($\approx 0.16$--$0.24$ for every mechanism tried so far).
Interpretation: aggressive absolute clipping -- not calibration to the honest ceiling
-- appears to be the dominant robustness lever here, and the earlier "cap $=$ honest
ceiling, barely touch honest gradients" framing (fixed-21, \texttt{gradclip\_calib},
\texttt{layerclip}) and this "clip hard, absolute anchor" framing may be doing
genuinely different things. \textbf{Caveat}: GeometricMedian only, 3 seeds -- the
$\gamma=0.33$ cells are a strong, consistent signal but not yet confirmed on
CenteredClipping/TrimmedMean/MultiKrum or at 5 seeds. Figure paths assume the plot
tree is reachable as below; adjust to your compile working directory if needed.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.48\textwidth]{../results/activation_clip_plots/cnn_mnist_selfclip_w1_gm3/best_test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
    \includegraphics[width=0.48\textwidth]{../results/activation_clip_plots/cnn_mnist_selfclip_w50_gm3/best_test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
    \caption{Self-calibrated warmup clip, GM only, 3 seeds. Left: $W=1$ (clip to the
    first gradient). Right: $W=50$ (cap $\approx$ honest ceiling). \texttt{best\_test}
    test accuracy over heterogeneity $\gamma$ (rows) and Byzantine count $f$
    (columns).}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/best_test_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:best_overall_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/best_test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:best_overall_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/best_test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:best_overall_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/best_test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:best_alie_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/best_test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:best_alie_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/best_test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:best_alie_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/best_test_SignFlipping_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:best_sf_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/best_test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:best_sf_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/best_test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:best_sf_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/best_test_Optimal_InnerProductManipulation_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:best_ipm_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/best_test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:best_ipm_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/best_test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:best_ipm_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:cc_worst_case_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:cc_worst_case_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:cc_worst_case_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:cc_alie_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:cc_alie_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:cc_alie_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_SignFlipping_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:cc_sf_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:cc_sf_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:cc_sf_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:cc_ipm_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:cc_ipm_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_CenteredClipping_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:cc_ipm_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:gm_worst_case_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:gm_worst_case_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:gm_worst_case_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:gm_alie_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:gm_alie_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:gm_alie_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_SignFlipping_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:gm_sf_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:gm_sf_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:gm_sf_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:gm_ipm_ramp_2}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:gm_ipm_qclip_080}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_GeometricMedian_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:gm_ipm_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:mk_worst_case_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:mk_worst_case_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:mk_worst_case_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:mk_alie_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:mk_alie_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:mk_alie_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_SignFlipping_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:mk_sf_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:mk_sf_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:mk_sf_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:mk_ipm_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:mk_ipm_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_MultiKrum_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:mk_ipm_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:tm_worst_case_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:tm_worst_case_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:tm_worst_case_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:tm_alie_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:tm_alie_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_ALittleIsEnough_neg1_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:tm_alie_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_SignFlipping_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:tm_sf_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:tm_sf_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_SignFlipping_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:tm_sf_rawqclip_080}
    \end{subfigure}
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_clip_ramp_2/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_clip_ramp_2_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Ramp $C{=}2$ ($r{=}2$)}
        \label{fig:tm_ipm_ramp_2}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../activation_clip_plots/cnn_mnist_qclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (momentum)}
        \label{fig:tm_ipm_qclip_080}
    \end{subfigure}
    \vspace{0.3cm}
    \begin{subfigure}[b]{0.32\textwidth}
        \centering
        \includegraphics[width=\textwidth]{../results/activation_clip_plots/cnn_mnist_rawqclip_080/test_Optimal_InnerProductManipulation_mnist_cnn_mnist_gamma_similarity_niid_NNM_ARC_TrMean_nb_honest_clients_10_tolerated_f_equal_real.pdf}
        \caption{Norm Clip $\tau{=}0.80$ (raw, 2 seeds)}
        \label{fig:tm_ipm_rawqclip_080}
    \end{subfigure}
