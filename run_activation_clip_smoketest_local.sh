...
#      clip with the STE backward (cnn_mnist_clip_qcoord_ste_080).
...
echo "[$(date)] Smoke test 1/2: adaptive per-coordinate quantile clip (STE, tau=0.8)"
python run_activation_clip_sweep.py \
  --config configs/activation_clip/_smoketest_qcoord_ste_local.json \
  --nb_jobs 1
