...
PLOTS_DIR = "./snn_benchmark_plots_cc_krum"
...
def generate_comparison_plots(nb_steps, evaluation_delta):
    print("\n==========================================================")
    print("GENERATING SWEEP COMPARISON PLOTS (CenteredClipping vs Krum)")
    print("==========================================================")
    ...
    for dist in distributions:
        ...
        ax.set_title(f"CenteredClipping vs Krum Sweep - MNIST ({title_dist})", fontweight="bold", pad=15)
        ...
        ax.set_title(f"CC vs Krum Learning Curves (f=5) - MNIST ({title_dist})", fontweight="bold", pad=15)
...
def main():
    parser = argparse.ArgumentParser(description="Run SNN Byzantine benchmark sweeps with CenteredClipping and Krum.")
    ...
    # Generate configurations for CenteredClipping and Krum
    cc_f0, cc_attacks = make_sweep_configs("snn_mnist_direct.json", "CenteredClipping", nb_steps, evaluation_delta)
    krum_f0, krum_attacks = make_sweep_configs("snn_mnist_direct.json", "Krum", nb_steps, evaluation_delta)
    
    configs_to_run = [
        ("snn_mnist_sweep_cc_f0.json", cc_f0),
        ("snn_mnist_sweep_cc_attacks.json", cc_attacks),
        ("snn_mnist_sweep_krum_f0.json", krum_f0),
        ("snn_mnist_sweep_krum_attacks.json", krum_attacks)
    ]
    ...
    print("Starting Sweep execution (CenteredClipping vs Krum)...")
    try:
