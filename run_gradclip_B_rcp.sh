#!/usr/bin/env bash
#
# POD B of 2 (fixed grad-norm clip study). Runs ALL 5 clip configs but only on the
# MORE-heterogeneous half of the grid (gamma in {0.33, 0.0}). POD A does the same
# 5 configs on gamma in {1.0, 0.66}. Each pod = 5 mechanisms x 2 gammas = exactly
...
# (--no_plots); regenerate them once after BOTH pods finish with:
#   bash run_gradclip_plots.sh
#
# Run inside the RCP container from the repo root. Requires a REBUILT+PUSHED image.
# Usage:  bash run_gradclip_B_rcp.sh [nb_jobs]
...
echo "========================================="
echo "[$(date)] Grad-clip POD B -- gammas {${GAMMAS}} (nb_jobs=${NB_JOBS})"
echo "========================================="
for config in "${CONFIGS[@]}"; do
  tag=$(basename "${config}" .json)
  log_file="results/logs/gradclip_${tag}_B.log"
...
echo "[$(date)] POD B complete. (Run run_gradclip_plots.sh after POD A also finishes.)"
