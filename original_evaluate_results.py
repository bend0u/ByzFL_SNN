...
# COMMENT: In the updated version, target_attack=None is added as a parameter:
...
def loss_heatmap(path_to_results, path_to_plot):
    ...
    attacks = ensure_list(attacks)
    # COMMENT: In the updated version, target_attack filtering is applied here:
                        ...
                        for y, nb_byzantine in enumerate(actual_nb_byz):
                            ...
                            for x, dist_param in enumerate(distribution_parameter_list):
                                ...
                                for attack in attacks:
                                    ...
                                    # COMMENT: Here we compute np.max(tab_acc), which extracts the peak accuracy 
                                    # reached at ANY point during training. This explains why non-convergent, 
                                    # noisy runs for f=8 sometimes show a higher value than f=6 (due to transient, 
                                    # random-walk peaks rather than true long-term convergence).
                        ...
                        for y, nb_byzantine in enumerate(actual_nb_byz):
                            ...
                            for x, dist_param in enumerate(distribution_parameter_list):
                                ...
                                for attack in attacks:
                                    ...
                                    tab_acc = tab_acc.mean(axis=0)
                                    # COMMENT: Here we extract the peak validation accuracy via np.max(tab_acc).
                                    # If a run is highly unstable/divergent (e.g. f=8 in noisy, non-iid settings),
                                    # a random-walk spike will be selected as the "accuracy", masking the fact
                                    # that the model failed to converge.
                                    accuracy = np.max(tab_acc)
