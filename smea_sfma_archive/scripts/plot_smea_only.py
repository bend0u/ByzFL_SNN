...
attacks = ['ALittleIsEnough', 'SignFlipping', 'InnerProductManipulation']
...
for idx, attack in enumerate(attacks):
    ...
    curve = get_mean_test_acc('results/smea_experiment/snn_atan/', pattern)
    
    if curve is not None:
        x_vals = np.arange(len(curve)) * 50
        ax.plot(x_vals, curve, label=attack, color=colors[idx], linewidth=2.5, marker='o', markersize=6)
...
save_path = '/localhome/bendouro/.gemini/antigravity-ide/brain/881c0a4f-bb2b-4e36-be90-1f4adf0775dd/smea_only_test_accuracy.png'
plt.savefig(save_path, dpi=300)
print(f'SMEA only plot saved to {save_path}')
